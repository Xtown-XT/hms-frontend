...
export default function EditRecord() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await editRecordService.getAll();
  ...
  const statusBadge = (status) => {
    ...
    switch (status) {
      ...
      case "OD": return `${base} bg-yellow-500 text-white`;
      case "CL": return `${base} bg-blue-500 text-white`;
      case "SL": return `${base} bg-purple-500 text-white`;
    ...
    Object.values(attendance).forEach((status) => {
      switch (status) {
        ...
        case "PR":
          ...
          break;
        case "A": totals.absentDays += 1; break;
        case "CL":
        case "SL":
        case "L":
          totals.leaveDays += 1;
                          ...
                          <td key={d} className="px-1 py-2 text-center">
                            {isEditing ? (
                              <select
                                ...
                                onChange={(e) => handleAttendanceChange(s.id, d, e.target.value)}
                                className="text-xs rounded border px-1 py-0.5"
                              >
                                {["P","A","L","OD","CL","SL","PR"].map((st) => (
                                  <option key={st} value={st}>{st}</option>
                                ))}
                              </select>
                            ) : (
                              <span className={statusBadge(s.attendance[d])}>{s.attendance[d]}</span>
                            )}
                          </td>
                        ))}
                        <td className="px-4 py-3 text-center">{totals.totalHours.toFixed(1)}</td>
                        <td className="px-4 py-3 text-center">
                          {isEditing ? (
                            <>
                              <button
                                className="text-green-600 font-semibold mr-2"
                                onClick={() => { setEditingRow(null); setToast("✔ Changes saved"); }}
                              >
                                Save
                              </button>
                              <button
                                className="text-red-600 font-semibold"
                                onClick={() => { setEditingRow(null); setStaffData(prev => prev.map(st => st.id === backupRow.id ? backupRow : st)); }}
                              >
                                Cancel
                              </button>
                            </>
                            ...
                            <button
                              ...
                              onClick={() => { setBackupRow(JSON.parse(JSON.stringify(s))); setEditingRow(s.id); }}
                            >
                              Edit
                            </button>
                          )}
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-80">
            ...
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setPrModal({ visible: false, staffId: null, day: null, fromTime: "", toTime: "" })}
                className="px-4 py-2 bg-gray-300 rounded"
              >
                Cancel
              </button>
              <button onClick={confirmPrHours} className="px-4 py-2 bg-indigo-600 text-white rounded">
                OK
              </button>
            </div>
          </div>
